...
## @param: filename                                        :    filename string, bytes                          
class FileInput:
    def __init__(self, filename_length, filename):
        self.filename_length = filename_length
        self.filename = filename

    def __str__(self):
        ret_str = ""
        ret_str += "filename_length:\n"
        ret_str += str(self.filename_length) + "\n"
        ret_str += "filename:\n"
        ret_str += str(self.filename) + "\n"
...
def peer_to_app_FileInput(peer_struct):
    filename_length = peer_struct.filename_length
    filename = peer_struct.filename
    return FileInput(filename_length, filename)

def app_to_peer_FileInput(app_struct):
    return antaris_api_pb2.FileInput(filename_length = app_struct.filename_length, filename = app_struct.filename)
...
## @param: correlation_id                                  :    correlation id for matching requests with responses and callbacks
## @param: app_id                                          :    App id for nowing the source of responses       
## @param: command_id                                      :    command id                                      
## @param: req_status                                      :    status of SatOS PA Message request              
class RespSatOsPaMsg:
    def __init__(self, correlation_id, app_id, command_id, req_status):
        self.correlation_id = correlation_id
        self.app_id = app_id
        self.command_id = command_id
        self.req_status = req_status
...
def peer_to_app_RespSatOsPaMsg(peer_struct):
    correlation_id = peer_struct.correlation_id
    app_id = peer_struct.app_id
    command_id = peer_struct.command_id
    req_status = peer_struct.req_status
    return RespSatOsPaMsg(correlation_id, app_id, command_id, req_status)

def app_to_peer_RespSatOsPaMsg(app_struct):
    return antaris_api_pb2.RespSatOsPaMsg(correlation_id = app_struct.correlation_id, app_id = app_struct.app_id, command_id = app_struct.command_id, req_status = app_struct.req_status)

## @class: HostToPeerFcmOperation
## @brief: To start or file copy from PS To ES
## @param: correlation_id                                  :    correlation id for matching requests with responses and callbacks
## @param: peer_app_id                                     :    PC appliction id from which files are copied to edge system
## @param: fcm_src                                         :    source of fcm command                           
## @param: fcm_dest                                        :    destination of fcm command                      
## @param: no_of_files                                     :    no of files to copy, if the value is 0 copy all files
## @param: file_input                                      :    details of Files to be copied                   
class HostToPeerFcmOperation:
    def __init__(self, correlation_id, peer_app_id, fcm_src, fcm_dest, no_of_files, file_input):
        self.correlation_id = correlation_id
        self.peer_app_id = peer_app_id
        self.fcm_src = fcm_src
        self.fcm_dest = fcm_dest
    ...
    def __str__(self):
        ...
        ret_str += str(self.correlation_id) + "\n"
        ret_str += "peer_app_id:\n"
        ret_str += str(self.peer_app_id) + "\n"
        ret_str += "fcm_src:\n"
        ret_str += str(self.fcm_src) + "\n"
        ret_str += "fcm_dest:\n"
        ret_str += str(self.fcm_dest) + "\n"
        ret_str += "no_of_files:\n"
        ret_str += str(self.no_of_files) + "\n"
        ret_str += "file_input:\n"
        ret_str += str(self.file_input) + "\n"
...
def peer_to_app_HostToPeerFcmOperation(peer_struct):
    correlation_id = peer_struct.correlation_id
    peer_app_id = peer_struct.peer_app_id
    fcm_src = peer_struct.fcm_src
    fcm_dest = peer_struct.fcm_dest
    no_of_files = peer_struct.no_of_files
    file_input = peer_struct.file_input
    return HostToPeerFcmOperation(correlation_id, peer_app_id, fcm_src, fcm_dest, no_of_files, file_input)

def app_to_peer_HostToPeerFcmOperation(app_struct):
    return antaris_api_pb2.HostToPeerFcmOperation(correlation_id = app_struct.correlation_id, peer_app_id = app_struct.peer_app_id, fcm_src = app_struct.fcm_src, fcm_dest = app_struct.fcm_dest, no_of_files = app_struct.no_of_files, file_input = app_struct.file_input)

## @class: HostToPeerFcmOperationNotify
## @brief: To send Notification to Es on completion of Fcm
## @param: correlation_id                                  :    correlation id for matching requests with responses and callbacks
## @param: fcm_complete                                    :    0 indiacates if all file transfer complete, -1 if files transfer in still ongoing 
## @param: req_status                                      :    status of file transfer                         
## @param: file_name                                       :    file_name, string                               
class HostToPeerFcmOperationNotify:
    def __init__(self, correlation_id, fcm_complete, req_status, file_name):
        self.correlation_id = correlation_id
        self.fcm_complete = fcm_complete
        self.req_status = req_status
    ...
    def __str__(self):
        ...
        ret_str += "correlation_id:\n"
        ret_str += str(self.correlation_id) + "\n"
        ret_str += "fcm_complete:\n"
        ret_str += str(self.fcm_complete) + "\n"
        ret_str += "req_status:\n"
        ret_str += str(self.req_status) + "\n"
        ret_str += "file_name:\n"
        ret_str += str(self.file_name) + "\n"
...
def peer_to_app_HostToPeerFcmOperationNotify(peer_struct):
    correlation_id = peer_struct.correlation_id
    fcm_complete = peer_struct.fcm_complete
    req_status = peer_struct.req_status
    file_name = peer_struct.file_name
    return HostToPeerFcmOperationNotify(correlation_id, fcm_complete, req_status, file_name)

def app_to_peer_HostToPeerFcmOperationNotify(app_struct):
    return antaris_api_pb2.HostToPeerFcmOperationNotify(correlation_id = app_struct.correlation_id, fcm_complete = app_struct.fcm_complete, req_status = app_struct.req_status, file_name = app_struct.file_name)
...
## @param: correlation_id                                  :    correlation id for matching requests with responses and callbacks
## @param: ac_app_id                                       :    Remote AC app_id                                
## @param: power_status                                    :    0: ON, 1 = OFF                                  
class NtfRemoteAcPwrStatus:
    def __init__(self, correlation_id, ac_app_id, power_status):
        self.correlation_id = correlation_id
        self.ac_app_id = ac_app_id
    ...
    def __str__(self):
        ...
        ret_str += str(self.correlation_id) + "\n"
        ret_str += "ac_app_id:\n"
        ret_str += str(self.ac_app_id) + "\n"
        ret_str += "power_status:\n"
        ret_str += str(self.power_status) + "\n"
...
def peer_to_app_NtfRemoteAcPwrStatus(peer_struct):
    correlation_id = peer_struct.correlation_id
    ac_app_id = peer_struct.ac_app_id
    power_status = peer_struct.power_status
    return NtfRemoteAcPwrStatus(correlation_id, ac_app_id, power_status)

def app_to_peer_NtfRemoteAcPwrStatus(app_struct):
    return antaris_api_pb2.NtfRemoteAcPwrStatus(correlation_id = app_struct.correlation_id, ac_app_id = app_struct.ac_app_id, power_status = app_struct.power_status)
...
def peer_to_app_PsTempReq(peer_struct):
    correlation_id = peer_struct.correlation_id
    return PsTempReq(correlation_id)

def app_to_peer_PsTempReq(app_struct):
    return antaris_api_pb2.PsTempReq(correlation_id = app_struct.correlation_id)
...
## @param: temperature                                     :    in Celsius                                      
class RespPsTemp:
    def __init__(self, correlation_id, temperature):
        self.correlation_id = correlation_id
        self.temperature = temperature
...
def peer_to_app_RespPsTemp(peer_struct):
    correlation_id = peer_struct.correlation_id
    temperature = peer_struct.temperature
    return RespPsTemp(correlation_id, temperature)

def app_to_peer_RespPsTemp(app_struct):
    return antaris_api_pb2.RespPsTemp(correlation_id = app_struct.correlation_id, temperature = app_struct.temperature)
...
class AntarisApiCallbackFuncList:
    def __init__(self, start_sequence, shutdown_app, process_health_check, process_response_register, process_response_get_current_location, process_response_stage_file_download, process_response_payload_power_control, req_payload_metrics, process_response_gnss_eph_stop, process_response_gnss_eph_start, process_cb_gnss_eph_data, process_response_get_eps_voltage_stop, process_response_get_eps_voltage_start, process_cb_get_eps_voltage, process_response_start_ses_therm_mgmnt_req, process_response_stop_ses_therm_mgmnt_req, process_response_ses_temp_req, process_cb_ses_thrml_ntf, process_pa_satos_msg_response, process_remote_ac_power_on_ntf, process_host_to_peer_fcm_operation_notify, process_satos_pa_msg, process_response_ps_temp_req):
        ...
        self.process_response_ps_temp_req = process_response_ps_temp_req
    ...
    def __str__(self):
        ...
        ret_str += "process_remote_ac_power_on_ntf:\n"
        ...
        ret_str += "process_response_ps_temp_req:\n"
        ret_str += str(self.process_response_ps_temp_req) + "\n"
...
def peer_to_app_AntarisApiCallbackFuncList(peer_struct):
    ...
    process_response_ps_temp_req = peer_struct.process_response_ps_temp_req
