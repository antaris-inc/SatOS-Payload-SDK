...
#define StageFile_Sequence_IDX          3
#define PaSatosMsg_ID                   "PaSatOsMsg"
#define PaSatosMsg_IDX                  4
#define SatosPaMsg_ID                   "SatOsPaMsg"
#define SatosPaMsg_IDX                  5
...
void handle_StageFile(mythreadState_t *mythread)
{
    ...
    StageHmData file_info;
    memcpy(&file_info.submod_id,&mythread->seq_params[0],4);
...
AntarisReturnCode start_sequence(StartSequenceParams *start_seq_param)
{
    ...
    printf("1: sub %d queue %d\n",start_seq_param->sequence_params[0], start_seq_param->sequence_params[1]);
...
AntarisReturnCode process_satos_pa_msg_reg(SatOsPaMsg *pa_satos_message)
{
   ...
   // printf("Command id = %hu , status = %d\n", resp_pa_satos_message->command_id, resp_pa_satos_message->req_status);
    ...
    file_info file;
    DataFile file_name;
    memcpy(&file,&pa_satos_message->payload_data[0],sizeof(file_info));
   // printf("submodule %d, queue id %d Data len %d\n",submoudle,queue_id,data_len);
...
AntarisReturnCode process_stage_hm_data_reg(StageHmData *hm_data_stage)
{
    AntarisReturnCode ret;
    char dir_path[256];
...
int main(int argc, char *argv[])
{
    ...
    AntarisApiCallbackFuncList callback_func_list = {
            ...
            process_stage_hm_data: process_stage_hm_data_reg,
    };
